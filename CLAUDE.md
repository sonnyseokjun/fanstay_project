...
- 2026-09-19 주제 변경: 이전 "중국인 대상 서울 한 달 살기" → "한국인 대상 해외 한 달 살기". 서비스 이름 FANSTAY는 그대로다.
...
- 폼 선택지의 `value` 코드는 백엔드 `signups/models.py`의 choices(`FEATURE_CHOICES` 포함)와 반드시 같아야 한다. "가 보고 싶은 곳"(백엔드 필드 `countries`)은 선택지 없이 직접 입력받는 문자열이다. 한쪽을 바꾸면 다른 쪽과 테스트, `plan.md` 11번을 함께 바꾼다.
