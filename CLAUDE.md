...
- 폼 선택지의 `value` 코드는 백엔드 `signups/models.py`의 choices(`FEATURE_CHOICES` 포함)와 반드시 같아야 한다. "가 보고 싶은 나라"는 선택지 없이 직접 입력받는 문자열이다. 한쪽을 바꾸면 다른 쪽과 테스트, `plan.md` 11번을 함께 바꾼다.
